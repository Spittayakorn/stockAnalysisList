...
$sdateTime = $_REQUEST['sdateTime'];
$edateTime = $_REQUEST['edateTime'];

//$sdateTime = '2021-06-29 13:13:40';
//$edateTime = '2021-06-29 22:05:40';
...
$sdateSpliteT = spliteT($sdateTime);
$edateSpliteT = spliteT($edateTime);
...
if ($sqlNumRowSelItem > 0) {
    ...
    while ($sqlFSelItem = mysqli_fetch_array($sqlQSelItem)) {
        ...
        //echo "$itemN$stockN ";
        $sqlSelDateTime = "select * from stockList where stockDate between '".$sdateSpliteT."' and '".$edateSpliteT."'  order by stockDate;";
                ...
                if ($sqlNumRowSqlStockAna > 0) {
                    while ($sqlFStockAna = mysqli_fetch_array($sqlQSelStockAna)) {
                        ...
                        if ($itemN == $sqlFStockAna[6]) {
                            ...
                            $takeN += floatval($sqlFStockAna[1]);
                            if ($j == 0) {
                                $exist = floatval($sqlFStockAna[2]);
                            }
                            $used = floatval($sqlFStockAna[3]);
                            $usedN += floatval($sqlFStockAna[3]);
        ...
        if ($checkSelItem == true) {
            ...
            if ($take == 0 && $existN == 0 && $used == 0 && $exist == 0 && $added == 0) {
                ...
                $record_json->take = $takeN;
                $record_json->exist = $existN;
                $record_json->used = $usedN;
